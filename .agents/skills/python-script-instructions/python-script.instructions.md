...
Conventions for Python 3.11+ scripts used in automation, tooling, and CLI
applications.
...
Standard exit codes: 0 success, 1 failure, 2 configuration error, 130 user
interrupt (SIGINT).
...
For complex CLIs with subcommands or interactive prompts, use the _click_
framework.
...
Use `@click.group()` for subcommands, `ctx.exit(code)` for exit codes, and
`ctx.fail(message)` for errors.
...
Create module-level logger, configure early in main. For file logging, add
_FileHandler_ to the root logger.

## Path Handling

Use _pathlib.Path_ exclusively; avoid _os.path_.
...
Common patterns: `cwd()`, `resolve()`, `exists()`, `is_dir()`, `is_file()`,
`iterdir()`, `glob()`, `rglob()`, `read_text()`, `write_text()`,
`mkdir(parents=True, exist_ok=True)`, `parent`, `name`, `stem`, `suffix`.

## Subprocess Execution

Use _subprocess.run()_ with error handling.
...
Use `capture_output=True` and `text=True` for string output. Use `check=True` to
...
Use `list[str]` not `typing.List[str]`, `str | None` not `Optional[str]`,
`Literal` for constrained values, `Self` for chained methods.
...
PEP 723 inline metadata enables automatic dependency installation with _uv_.
...
Place after copyright and SPDX headers, before module docstring. Run with
`uv run script.py`.
