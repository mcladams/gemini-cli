...
const SCRIPTS_DIR = join(process.cwd(), 'tools', 'gemini-cli-bot', 'metrics', 'scripts');
const SYNC_SCRIPT = join(process.cwd(), 'tools', 'gemini-cli-bot', 'history', 'sync.ts');
const OUTPUT_FILE = join(process.cwd(), 'tools', 'gemini-cli-bot', 'history', 'metrics-before.csv');
const TIMESERIES_FILE = join(process.cwd(), 'tools', 'gemini-cli-bot', 'history', 'metrics-timeseries.csv');
function processOutputLine(line, results) {
    ...
    if (metricName && !isNaN(metricValue)) {
        ...
        if (avg7d !== null) {
            results.push(`${metricName}_delta_7d,${(metricValue - avg7d).toFixed(2)}`);
        }
        const avg30d = getHistoricalAverage(metricName, 30);
        if (avg30d !== null) {
            results.push(`${metricName}_delta_30d,${(metricValue - avg30d).toFixed(2)}`);
        }
    }
}
async function run() {
    ...
    const scripts = readdirSync(SCRIPTS_DIR).filter((file) => file.endsWith('.ts') || file.endsWith('.js'));
    const results = ['metric,value'];
