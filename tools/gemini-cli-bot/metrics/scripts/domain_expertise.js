...
try {
  ...
  `;
    const output = execSync(`gh api graphql -F owner=${GITHUB_OWNER} -F repo=${GITHUB_REPO} -f query='${query}'`, { encoding: 'utf-8', stdio: ['ignore', 'pipe', 'ignore'] });
    ...
    const getAuthors = (targetPath) => {
        if (authorCache.has(targetPath))
            return authorCache.get(targetPath);
        try {
            const authors = execSync(`git log --format="%an|%ae" -- ${JSON.stringify(targetPath)}`, {
                cwd: repoRoot,
                encoding: 'utf-8',
                stdio: ['ignore', 'pipe', 'ignore'],
            }).toLowerCase();
    ...
    for (const pr of data.pullRequests.nodes) {
        ...
        if (!commitHash)
            continue; // Skip if we don't have the commit locally
    ...
    const ratio = totalMaintainerReviews > 0
...
//# sourceMappingURL=domain_expertise.js.map