  ...
  `;
    ...
    const getFirstResponseTime = (item) => {
        ...
        if (item.comments?.nodes)
            checkNodes(item.comments.nodes);
        if (item.reviews?.nodes)
            checkNodes(item.reviews.nodes);
        if (earliestResponse) {
            return ((earliestResponse - new Date(item.createdAt).getTime()) /
                (1000 * 60 * 60));
        }
...
//# sourceMappingURL=time_to_first_response.js.map